...
> prompt whose `registry.yaml` entry sets `about: true`. Keep it free of any
> individual's name, location, or other personal identifier.
>
> **This block is the SINGLE source of Clannon's identity** — the central
> orchestrator, every batch orchestrator, and every expert receive the same text, so
> there is one Clannon rather than a company of separate agents.
