...
| `/runs` | GET | `?projectId=<id>` (optional) | `RunSummary[]` — filtered to one project when `projectId` is given, all of the user's effective runs when omitted. Superseded turns never appear. Each `RunSummary`/`Run` carries an optional `projectId` |
