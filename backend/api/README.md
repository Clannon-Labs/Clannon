...
| `/runs` | GET | `?projectId=<id>` (optional) | `RunSummary[]` — filtered to one project when `projectId` is given, all of the user's effective runs when omitted. Superseded turns never appear. Each `RunSummary`/`Run` carries `tokensUsed`, `cacheReadTokens`, `cacheWriteTokens`, `startedAt`, `firstMessageAt`, `completedAt`, plus an optional `projectId` |
...
| `/runs/:id` | GET | — | full `Run` (decisionLog, experts, **message**, report, sources, artifacts, inputs, feedbackRating, parentRunId, sessionId, blockStage, verificationState, **completionState**, **completionReason**, `startedAt`, `firstMessageAt`, `completedAt`). `message` = the orchestrator's conversational chat bubble (the agent talking to the user); `report`/`artifacts` = the deliverable. A turn may have `message` only (pure conversation), both, or `report` only. Superseded run IDs return the same non-disclosing `404` as unknown/foreign IDs. `startedAt`/`firstMessageAt`/`completedAt` are nullable ISO timestamps: `startedAt` is set when `run_driver.execute()` begins the pipeline stage chain (so `createdAt`→`startedAt` is queue time, a distinct thing); `completedAt` is set at the terminal choke point (every terminal status, including blocked/failed/cancelled); `firstMessageAt` is set ONLY the first time the orchestrator's LIVE narration (`say()`, streamed as `message_delta` mid-run) reaches the client — deliberately NOT set by `report_delta` chunking or by the report-mode's direct final `message_delta` (both fire terminal-adjacent, a few hundred ms before completion, and would misrepresent "time to first value" as time-to-nearly-done). A report-mode run with no live narration can be `delivered` with real content and still have `firstMessageAt == null` — that absence is honest, not a bug. |
...
| `/usage` | GET | — | `UsageSummary` `{periodStart, periodEnd, periodEndExclusive:true, baseBudget, additionalCredits, budget, used, cacheReadTokens, cacheWriteTokens, byDay[], latency}`. Period is anchored to signup date or latest confirmed plan-payment date in UTC; missing month days clamp without drift (Jan 31 → Feb 28/29 → Mar 31). `periodEnd` is exclusive. `used` sums real `run.tokensUsed`, including superseded turns, only inside that fixed period. `budget = baseBudget + confirmed current-period credits`; pending/failed credits grant zero and unknown plans fail closed to zero. Cached counters remain separate. `latency` = `{inPeriod, timeToFirstMessageMs:{sampleSize,excluded,p50,p95}, totalDurationMs:{sampleSize,excluded,p50,p95}}` over this period's runs — TWO independent samples, each needing only its own pair of stamps: `totalDurationMs` needs `startedAt`+`completedAt`; `timeToFirstMessageMs` needs `startedAt`+`firstMessageAt` (see `/runs/:id`). A report-mode run can be `delivered` with real content and still have no `firstMessageAt` (no live narration is honest, not a bug — see `/runs/:id/stream`'s `message_delta` note) — it still counts toward `totalDurationMs`, just not `timeToFirstMessageMs`. Each sample's own `excluded = inPeriod - sampleSize`; missing `startedAt` (never started, or a pre-migration row) excludes a run from both. `p50`/`p95` are `null` (never `0`) when that sample is empty. This is a REST-only aggregate — no `usage` SSE event is emitted (see `/runs/:id/stream` below). |
