...
- It streams its structured decision log and sparse `say()` commentary DIRECTLY
  to the user, live; the final answer buffers through the output filter for both
  chat and report presentation. Never route the live log through the filter;
  never stream final-answer content before the filter accepts it.
...
- `orchestrator.py` = stage door (builds ports, applies the whole-turn timeout,
  ...
  map `OrchestratorAnswer` to `OrchestratorResponse`. `OrchestratorAnswer` has
  exactly `answer_text`, `presentation`, `confidence`, `deliverable_ref`;
  presentation is explicit intent, never inferred from `say()`, length, tools,
  or artifacts. Own contracts in `schemas.py`; ports in `ports.py`; internals in
  `utils/`.
- Bounds: `ORCHESTRATOR_MAX_TURNS` (cap forces a final answer), `ORCHESTRATOR_TIMEOUT_S`
  ...
  to an empty package with an honest decision-log note. Central and batch prompts
  are unlocked registry entries; batch definitions receive overlay-resolved prompt
  text from `registry.config.batches`, never inline fallback prose.

## Tests
`tests/orchestrator_*.py` (loop, ports, stage, experts, tools, registry, recovery)
plus `tests/prompt_overlay.py` and `tests/registry_config_batches.py`.
