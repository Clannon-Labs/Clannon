...
- Fail closed: if a gate can't run, reject. In prod a missing YARA rule set fails
  closed (`CLANNON_ENV` is canonical; legacy-only `VRAKSHA_ENV` remains a migration
  alias). `AGENT_REQUIRE_YARA=1` may tighten non-production mode, never loosen prod.
