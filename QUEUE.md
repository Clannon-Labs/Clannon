...
4. Task 7 — one signature-behavior characterization (the memory water-filling
   allocation), lower blast radius, so it rides on the safety nets the earlier tasks
   established. (The expert run() conformance test originally planned here was
   completed by the loop as PR #13 during this prep, so it was pulled to avoid a
   duplicate; this queue is 7 tasks, not 8.)
